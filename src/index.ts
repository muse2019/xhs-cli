...
// 读取 feed 缓存
function readFeedCache(): Array<{ id: string; title: string; author: string; url: string }> {
  try {
...
// 写入 feed 缓存
function writeFeedCache(feed: Array<{ id: string; title: string; author: string; url: string }>) {
  try {
...
function outputAgentResponse(response: AgentResponse, isAgent: boolean): void {
  if (isAgent) {
...
xiaohongshu
  ...
  .action(async (opts: { limit: string; json: boolean; agent: boolean }) => {
    const page = await getBridgePage();
...
xiaohongshu
  ...
  .action(async (num: string, opts: { agent: boolean }) => {
    ...
    if ((info as any).likes || (info as any).collects) {
      console.log(chalk.dim(`互动: ${(info as any).likes || 0} 赞 · ${(info as any).collects || 0} 收藏`));
    }
...
xiaohongshu
  ...
  .action(async (id: string | undefined, opts: { duration: string }) => {
    // 如果提供了 ID，先导航到笔记页面
    if (id) {
